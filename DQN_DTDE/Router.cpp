...
if(!this->sock_started){
    ...
    std::string myPidprocessLocation = "/home/bedreddine/Desktop/my-simulations/extensions/fw/DQN/processPIDs.txt";

    // Construire la commande pour démarrer agent.py et écrire son PID dans le fichier
    std::ostringstream agentCommand;
    agentCommand << "python3.8 /home/bedreddine/Desktop/my-simulations/extensions/fw/DQN/Agent.py "
                 << port << " " << selected_seed << " " << myScenario
                 << " & echo $! > " << pidFilename
                 << " && echo $! >> " << myPidprocessLocation;
    ...
    killerScriptCommand << "python3.8 /home/bedreddine/Desktop/my-simulations/extensions/fw/DQN/tuer.py "
...
void Router::Send_reward(std::vector<float>& State,int chosen_face,float reward, string function){  
